...
public class LocationService {
    ...
    public LocationService(Context locationContext) {
        ...
        synchronized (objLock) {
            if (client == null) {
                try {
                    //setAgreePrivacy接口需要在LocationClient实例化之前调用，如果setAgreePrivacy接口参数设置为了false，则会报：The location function has been stopped because you do not agree with the privacy compliance policy. Please recheck the setAgreePrivacy interface
                    LocationClient.setAgreePrivacy(true);
                    client = new LocationClient(locationContext);
                } catch (Exception e) {
                    e.printStackTrace();
                    Log.e("BaiduMapError",e.getMessage());
                }
            }
            if (client != null) {
    ...
    public LocationClientOption getDefaultLocationClientOption() {
        if (mOption == null) {
            ...
            mOption.setScanSpan(60 * 1000); // 可选，默认0，即仅定位一次，设置发起连续定位请求的间隔需要大于等于1000ms才是有效的
